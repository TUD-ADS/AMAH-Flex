...
proc synthesize { module } {
   ...
   command "create_project -in_memory -part $board_part" "$resultDir/create_project.log"
   ...
   # skip synthesis for netlist only designs
   if {[llength [get_property -quiet FILE_TYPE [get_files -quiet -filter  {FILE_TYPE != EDIF && FILE_TYPE != NGC}]]] == 0} {
	   puts "INFO: Skipping synthesis for this Module. The design contains only netlists. Linking the desing instead."
	   set start_time [clock seconds]
	   if {$topLevel} {
	   		error "ERROR: Using netlist only design as top is not yet supported."		   
	   }
	   command "link_design -mode out_of_context -top $moduleName -part $board_part" "$resultDir/${moduleName}_link_design.log"
	   set end_time [clock seconds]
	   log_time link_design $start_time $end_time 0 "$moduleName $options"
   } else {
       #### synthesis
       puts "\tRunning synth_design"
       set start_time [clock seconds]
       command "set_property top top_counter \[current_fileset\]"
       if {$topLevel} {
          command "synth_design -mode default $options -top $moduleName -part $board_part" "$resultDir/${moduleName}_synth_design.rds"
       } else {
          command "synth_design -mode out_of_context $options -top $moduleName -part $board_part" "$resultDir/${moduleName}_synth_design.rds"
       }
       set end_time [clock seconds]
   ...
   if {[lsearch $RM_modules ${module}] >= 0} {
       command "opt_design"
       command "report_utilization -hierarchical -file $resultDir/${module}_utilization_hierarchical_synth.rpt"
   } else {
		command "write_edif $resultDir/${module}.edf"
   }
